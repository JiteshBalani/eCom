...
//   const userId = useSelector((state) => state.auth.userId); // Assuming you have userId in your auth state
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className="shadow-md" title={<span className="font-semibold">Delivery Information</span>}>
          <Form layout="vertical" onFinish={(values) => handleOrder(values, 'cod')}>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-between mt-6">
              ...
              <Button
                ...
                onClick={() => message.info('Online payment not yet implemented')}
              >
                Pay Online
              </Button>
            </div>
        ...
        <Card className="shadow-md" title={<span className="font-semibold">Order Summary</span>}>
            ...
            {cartItems.map((item) => (
              <div key={item._id} className="flex items-center gap-4">
                ...
                <div className="text-right">
                  <p className="text-sm font-semibold">₹{item.salePrice * item.quantity}</p>
                  <p className="text-xs line-through text-gray-400">₹{item.mrp * item.quantity}</p>
                </div>
              </div>
            ))}
            <Divider />
